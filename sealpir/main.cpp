...
int main(int argc, char *argv[]) { // NOT use expand;

    uint64_t number_of_items = 1 << 12; // N = 4096
    ...
    // uint64_t ele_index = rd() % number_of_items; // element in DB at random position // 随机设定要查询的index；
    uint64_t ele_index = 4094;
    uint64_t index = client.get_fv_index(ele_index);   // index of FV plaintext // index转换为block_index; 多个数据表元素编排到一个明文
                                                        // client解密后时候用自己的offset便可以得到具备的值;
